...
**Rule: never use a native `<select>` for a styled dropdown.** Native `<option>` popups
cannot be styled (borderless rows, hover color, width/position) and clip inside
`overflow:auto` modals and panels. Use the shared custom listbox component (`SelectMenu`)
for every dropdown, and route label+control rows (`DropdownRow`) through it.
- Trigger (must match the text-input field exactly)
  - Layout: `display:flex; align-items:center; gap:var(--space-xs)`, `width:100%`, `height:44px`.
  - Type: `font-size:16px`, `font-family:var(--font-sans)`, `color:var(--color-ink)`.
  - Surface: `background:var(--color-canvas)`, `border:1px solid var(--color-hairline-strong)`,
    `border-radius:var(--rounded-md)`, `padding:var(--space-sm) var(--space-md)`.
  - Hover (enabled): `border-color:var(--color-stone)`; `transition:border-color 150ms ease`.
  - Open / `:focus-visible`: `border:2px solid var(--color-primary)` and reduce padding by 1px
    (`calc(var(--space-sm) - 1px) calc(var(--space-md) - 1px)`) so text never shifts.
  - Disabled: `background:var(--color-surface)`, `color:var(--color-muted)`, `cursor:not-allowed`.
  - Value text: `flex:1` with single-line ellipsis truncation; placeholder uses `var(--color-steel)`.
  - Caret: `chevron-down`, 16px, `var(--color-steel)`, rotates 180° when open
    (`transition:transform 150ms ease`).
- Popup (dimensions & position align seamlessly with the field)
  - Render teleported to `<body>` with `position:fixed; z-index:1000` (above modals).
  - Geometry is computed from the trigger's bounding rect every time it opens and on
    scroll/resize: `left` and `width` equal the trigger's; place 4px below it; flip above
    when space below `< 200px` and more room exists above. Clamp height to
    `max(140px, min(300px, available))` with `overflow-y:auto`.
  - Container: `background:var(--color-canvas)`, `border:1px solid var(--color-hairline)`,
    `border-radius:var(--rounded-md)`, `box-shadow:var(--shadow-modal)`,
    `padding:var(--space-xxs)`, `gap:1px`, 120ms fade-in.
  - Close on outside-click, `Esc`, and `Tab`.
- Options (dynamic, borderless)
  - `border:none; background:transparent`, `border-radius:var(--rounded-sm)`,
    `padding:9px var(--space-sm)`, `transition:background-color 120ms ease, color 120ms ease`.
  - Font auto-matches the field (`16px`, `var(--font-sans)`) — never the OS option font.
    Long labels ellipsize to fit and expose full text via the `title` attribute; never wrap,
    never overflow.
  - Hover / keyboard-active: background `rgba(0,117,222,0.10)` (transparent light blue),
    text `var(--color-link-blue-pressed)`.
  - Selected row: `font-weight:600`, `color:var(--color-link-blue)`, trailing `check` icon;
    on hover its fill deepens to `rgba(0,117,222,0.14)`.
  - Disabled option: `color:var(--color-muted); cursor:not-allowed`; excluded from hover/active.

  > The translucent-blue hover values are component-scoped literals, not `:root` tokens
  > (the design-token block stays the single source of truth per DESIGN.md).
- API & accessibility (parity with native `<select>`)
  - Props: `v-model` (string) · `options: { value, label, disabled? }[]` · `placeholder` · `disabled`.
  - A real empty-string choice stays in `options`; a non-selectable prompt uses `placeholder`.
  - Semantics: `aria-haspopup="listbox"` + `aria-expanded` on the trigger; `role="listbox"` on
    the popup; `role="option"` + `aria-selected` per row.
  - Keyboard: `↑`/`↓` move active (skipping disabled), `Enter`/`Space` select, `Esc` closes;
    focus returns to the trigger after selection.

### Sidebar top-level (group) icons

**Rule: every top-level / parent nav item has a unique, theme-aligned icon.** Never repeat a generic placeholder (e.g. `box`) across multiple groups.
- Icons come from the single Lucide outline family via `AppIcon` (16px in nav,
  `stroke-width:1.6`); register a semantic key in `AppIcon.vue` before use. No mixing families.
- Choose a glyph that depicts the group's domain, distinct from sibling groups (and, where
  practical, from its own child items).
